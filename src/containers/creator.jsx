...
import { ContainerWrapped, TextWrapped, IconWrapped, ImageWrapped,PlaceholderWrapped } from '../components/index';
...
import { defaultStyle, defaultProps, oftenStyle, baseComponents } from '../data/defaultStyle';
...
let loadComponent = function (id, name, style, props, children) {
    switch (name) {
        ...
        case 'Placeholder':
            return <PlaceholderWrapped key={id} style={style} {...props} />;
    }
...
class Index extends React.Component {
    ...
    render() {
        ...
        let {styleList, elementList, propsList, componentList} = this.props;
                ...
                <div style={{ display: 'flex', overflow: 'auto' }}>
                    <div style={{ width: 315 }}>
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex' }}>
                                {baseComponents.map((basecomV) =>
                                    <ComponentType key={basecomV.name} active={this.state.active == basecomV.name} text={basecomV.remark} onClick={() => { this.setState({ active:  basecomV.name }) }} />
                                )}
                            </div>
                            <h2>custom Components</h2>
                            <div style={{ display: 'flex' }}>
                                {componentList.map((componentV) =>
                                    <ComponentType key={componentV.name} active={this.state.active == componentV.name} text={componentV.name} onClick={() => { this.setState({ active: componentV.name }) }} />
                                )}
                            </div>
                            <h2>Add</h2>
                            <div style={{ display: 'flex' }}>
                    ...
                    <div style={{ width: 315 }}>
                        ...
                        <div>
                            ...
                            <div>
                                {oftenStyle[activeElement.name].map((style, index) => <div key={style.name}>
                        ...
                        <div>
                            <div>save</div>
                            <div>
                                <div><a href='javascript:;' onClick={()=>{this.props.SaveData()}}>save</a></div>
                            </div>
                        </div>
                    </div>
                </div>
